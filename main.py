...
def analysis():
    ...
    df = sales_train_df[sales_train_df['item_price'] > 0]
    df = df.sort_values(by=['shop_id'])
    fig = px.scatter(df, x='item_price', y='date_block_num', facet_col='shop_id',
                     facet_col_wrap=7, facet_row_spacing=0.125)
    fig.show()
